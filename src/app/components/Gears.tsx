      ...
      <div className="px-6 lg:px-12 py-6 lg:py-8">
        ...
        <div className="flex items-end justify-between mb-6 lg:mb-8 gap-4 flex-wrap">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="px-2 py-0.5 text-[10px] tracking-[0.2em]" style={{ ...mono, backgroundColor: INK, color: BONE }}>
                ARSENAL // 04
              </span>
              <span className="text-[11px] tracking-[0.2em]" style={mono}>LUFFY // GEAR TRANSFORMATIONS</span>
            </div>
            <h1
              className="text-7xl sm:text-8xl lg:text-[10rem] leading-[0.82] uppercase"
              style={{ ...display, color: INK }}
            >
              Luffy<span style={{ color: active.accent, textShadow: `4px 4px 0 ${INK}` }}>'s</span> Gears
            </h1>
          </div>
          <div className="text-right">
            <div className="text-[10px] tracking-[0.25em]" style={mono}>BOUNTY</div>
            <div className="text-4xl lg:text-5xl" style={{ ...display, color: active.accent, textShadow: `2px 2px 0 ${INK}` }}>
              ฿ 3,000,000,000
            </div>
          </div>
        </div>
          ...
          {gears.map((g) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-sm tracking-[0.2em]" style={display}>GEAR {g.num}</span>
              </motion.button>
            );
          })}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-stretch">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {/* Left vertical LUFFY */}
              <div className="absolute left-4 top-4 bottom-4 flex items-start" style={{ writingMode: 'vertical-rl' }}>
                <span
              ...
              <div
                ...
                style={{ ...jp, color: active.accent, fontWeight: 900, textShadow: `4px 4px 0 ${INK}` }}
              >
                {active.jp}
              </div>
              {/* Tagline */}
              <div className="absolute bottom-6 right-6 text-right max-w-xs">
                <div className="text-[10px] tracking-[0.3em] mb-1" style={mono}>// FORM</div>
                <div className="text-3xl lg:text-4xl uppercase" style={{ ...display, color: INK }}>
                  {active.tagline}
                </div>
              </div>
              {/* Placeholder tag */}
          ...
          <div className="lg:col-span-4 flex flex-col gap-4">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                style={{ backgroundColor: BONE, border: `2px solid ${INK}`, boxShadow: `5px 5px 0 ${INK}` }}
              >
                <div className="flex items-center justify-between mb-3">
                  <span className="px-1.5 py-0.5 text-[10px] tracking-[0.2em]" style={{ ...mono, backgroundColor: INK, color: BONE }}>DATA FILE</span>
                  <span className="text-[10px]" style={mono}>FILE_NO.GEAR{active.num}</span>
                </div>
                <h2 className="text-4xl lg:text-5xl leading-none uppercase mb-1" style={display}>{active.name}</h2>
                <div className="text-xl mb-3" style={{ ...jp, color: active.accentDeep, fontWeight: 900 }}>{active.jp}</div>
                <p className="text-sm lg:text-base mb-4">{active.desc}</p>
                <div className="h-px w-full mb-3" style={{ backgroundColor: INK, opacity: 0.2 }} />
                <div className="grid grid-cols-2 gap-y-1.5 text-xs" style={mono}>
                  ...
                  <span className="text-right" style={{ color: active.accent, fontWeight: 700 }}>{active.model}</span>
                  <span className="opacity-60">UNLOCKED</span>
                  <span className="text-right">{active.unlocked}</span>
                  <span className="opacity-60">STATUS</span>
                  <span className="text-right" style={{ color: active.accent }}>● AWAKENED</span>
                </div>
              </motion.div>
            </AnimatePresence>
            ...
            <AnimatePresence mode="wait">
              <motion.div
                ...
                style={{ backgroundColor: active.accent, color: BONE, border: `2px solid ${INK}`, boxShadow: `5px 5px 0 ${INK}` }}
              >
                <div className="text-[10px] tracking-[0.3em] opacity-80 mb-2" style={mono}>// SUN GOD NIKA</div>
                <p className="text-xl lg:text-2xl leading-snug uppercase" style={display}>
                  "I'm gonna be King of the Pirates!"
                </p>
                <div className="text-[10px] tracking-[0.25em] mt-3 opacity-80" style={mono}>— MONKEY D. LUFFY</div>
              </motion.div>
            </AnimatePresence>
          </div>
        </div>
